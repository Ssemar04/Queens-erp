...
type FilterTab = "all" | "unread" | "stock" | "orders" | "system";

const TAB_FILTER: Record<FilterTab, (n: Notification) => boolean> = {
  all: () => true,
  unread: (n) => !n.isRead,
  stock: (n) => n.type === "low_stock" || n.type === "zero_stock",
  orders: (n) => n.type === "po_reminder" || n.type === "po_overdue" || n.type === "request_update",
  system: (n) => n.type === "system",
};

const EMPTY_LABELS: Record<FilterTab, string> = {
  all: "No notifications",
  unread: "All caught up",
  stock: "No stock alerts",
...
export function NotificationCenter({ open, onOpenChange, onOpenPrefs }: NotificationCenterProps) {
  ...
  const handleClick = (n: Notification) => {
    ...
    if (n.link) {
      onOpenChange(false);
      const [to, queryString = ""] = n.link.split("?");
      const search = Object.fromEntries(new URLSearchParams(queryString));
      navigate({ to: to as "/app/dashboard", search });
    }
  };
        ...
        <div className="border-b border-border px-4 py-2">
          <Tabs value={tab} onValueChange={(v) => setTab(v as FilterTab)}>
            <TabsList className="h-8 w-full">
              ...
              <TabsTrigger value="orders" className="text-xs flex-1">Orders</TabsTrigger>
              <TabsTrigger value="system" className="text-xs flex-1">System</TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
        ...
        <ScrollArea className="flex-1">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-2 py-16 text-muted-foreground">
              ...
              <p className="text-sm">{EMPTY_LABELS[tab]}</p>
            </div>
          ) : (
