...
import type { Category, Supplier, Branch } from "@/types/inventory";

// Keep compatibility, use local ItemFilters interface
...
export function CatalogFilters({ filters, onChange, categories, suppliers, branches, onAddCategory }: CatalogFiltersProps) {
  const [mobileOpen, setMobileOpen] = useState(false);
  const [categoryOpen, setCategoryOpen] = useState(false);

  const activeCount = [filters.categoryId, filters.supplierId, filters.status, filters.branchId, filters.search].filter(Boolean).length;
        ...
        <SelectContent>
          ...
          {onAddCategory && (
            <div className="border-t border-border p-1">
              <button
                type="button"
                className="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-left text-sm text-primary outline-none hover:bg-accent"
                onMouseDown={(e) => {
                  e.preventDefault();
