...
function validateRows(
  ...
  const branchSet = new Set(knownBranches.map((b) => b.toLowerCase()));
  ...
  return mappedRows.map((row) => {
    ...
    // Category / supplier / branch warnings
    ...
    if (branch && !branchSet.has(branch.toLowerCase())) {
      warnings.push(`New branch: "${branch}"`);
    }
...
export function CSVImportSheet({
  ...
  const validatedRows = useMemo(() => {
    if (step < 3) return [];
    return validateRows(mappedRows, fields, existingSkus, knownCategories, knownSuppliers, knownBranches);
  }, [step, mappedRows, fields, existingSkus, knownCategories, knownSuppliers, knownBranches]);
